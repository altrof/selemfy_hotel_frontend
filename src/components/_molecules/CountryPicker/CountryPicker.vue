      ...
      <span class="inline-block w-full rounded-md shadow-sm">
        ...
        <div v-show="isOpen" class="absolute mt-1 w-full rounded-md bg-white shadow-lg">
          <ul tabindex="-1" role="listbox" aria-labelledby="listbox-label" aria-activedescendant="listbox-item-3" class="max-h-56 rounded-md py-1 text-base leading-6 shadow-xs overflow-auto focus:outline-none sm:text-sm sm:leading-5">
            ...
            -->
            <li tabindex="0" @click="select(country)" id="listbox-item-0" role="option" v-for="country in countries" v-bind:key="country" class="text-gray-900 cursor-default select-none relative py-2 pl-3 pr-9  cursor-pointer hover:text-white hover:bg-indigo-600 focus:outline-none focus:text-white focus:bg-indigo-600">
              <div class="flex items-center space-x-3">
                <img :src="`${country.flagSvg}`" alt="" class="flex-shrink-0 h-6 w-6 rounded-full" />
                <!-- Selected: "font-semibold", Not Selected: "font-normal" -->
                <span class="block truncate" v-bind:class="{ 'font-normal' : !isSelected(country) , 'font-semibold' : isSelected(country)}">
                {{ country.name }}
              </span>
              </div>
              ...
              -->
              <span v-show="isSelected(country)" class="absolute inset-y-0 right-0 flex items-center pr-4">
