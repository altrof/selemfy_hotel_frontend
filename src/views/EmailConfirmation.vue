...
onMounted(async () => { await AuthAPI.confirmEmailToken(token)
...
<div class="flex h-screen w-screen place-items-center justify-center pb-14">
  <p
    v-if="responseData?.status === 500 && responseData?.message === 'Token is expired'"
    class="border border-red-900 px-14 py-10 border-l-8 border-l-red-500 bg-red-200/30"
  >
    {{ responseData.message }}
  </p>
  <p
    v-else-if="responseData?.status === 500 && responseData?.message === 'Email is already confirmed!'"
